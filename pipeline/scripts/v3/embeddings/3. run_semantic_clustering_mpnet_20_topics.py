...
"""
TRISS V3 Pipeline: Forced-20-Topic Semantic Clustering with MPNet (TEST ONLY)
=============================================================================
Equivalent of '3. run_semantic_clustering_10_topics.py' but:
  - Uses MPNet (all-mpnet-base-v2) researcher embeddings instead of OpenAI
  - Forces exactly K=20 clusters
  - TRISS-level only (no school-level breakdown)
  - Writes all output to: 1. data/5. analysis/global/topic_mpnet_20/

Purpose: compare how MPNet embedding space clusters vs OpenAI at the
same granularity (K=20).

Outputs in topic_mpnet_20/:
  global_cluster_assignments.csv   — researcher → cluster ID
  global_cluster_centroids.npy     — (20, D) centroid matrix
...
OUT_DIR    = BASE / "1. data/5. analysis/global/topic_mpnet_20"

FORCED_K     = 20          # forced number of clusters
MODEL        = "mpnet"     # ← MPNet embeddings
RANDOM_STATE = 42
...
def main():
    ...
    # 2. Load MPNet researcher embeddings
    ...
    print(f"  {len(df_users)} researchers loaded. Embedding dim: {df_users['embedding'].iloc[0].shape[0]}")

    X = np.vstack(df_users["embedding"].values)

    # 3. Forced K=20 KMeans
    print(f"\nRunning KMeans with K={FORCED_K} (MPNet, forced — no silhouette search)...")
    ...
    # 6. LLM descriptions
    print(f"\nGenerating LLM descriptions for {FORCED_K} topics (via GPT-4o-mini)...")
    ...
    for cluster_id in tqdm(range(FORCED_K), desc="Topics"):
        ...
        print(f"  Topic {cluster_id:2d}: {desc.get('topic_name', '?')}")
